...
func doStruct(params interface{}, pointer interface{}, mapping ...map[string]string) (err error) {
	...
	for i := 0; i < pointerElemReflectValue.NumField(); i++ {
		...
		if !stringutil.IsLetterUpper(elemFieldType.Name[0]) {
			continue
		}
		...
		if elemFieldType.Anonymous {
			...
			attrMap[tempName] = stringutil.RemoveSymbols(tempName)
		}
	}
	if len(attrMap) == 0 {
		return nil
	...
	tagToNameMap, err := structutil.TagMapName(pointerElemReflectValue, StructTagPriority)
	if err != nil {
		return err
	}
	for k, v := range tagToNameMap {
		tagMap[v] = stringutil.RemoveSymbols(k)
	}
	...
	for mapK, mapV := range paramsMap {
		...
		if attrName == "" {
			checkName = stringutil.RemoveSymbols(mapK)
